...
def test_show_document_non_streaming(server_fixture: subprocess.Popen[str]) -> None:
    """Test showing a document non-streaming using A2A JSON-RPC protocol."""
    logger.info("Starting non-streaming show document test")

    message = Message(
        message_id=f"msg-user-{uuid.uuid4()}",
        role=Role.user,
        parts=[Part(root=TextPart(text="show gs://hcls-ge-documents/roi_2025_health.pdf"))],
    )
